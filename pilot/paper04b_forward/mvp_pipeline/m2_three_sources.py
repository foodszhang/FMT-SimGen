"""M2: Three-source single-view validation.

Run MCX simulation for all three source types and compare with closed-form.
Uses archived volume and coordinates for consistency with M1 validation.

Usage:
    uv run python m2_three_sources.py --n-photons 10000000
"""
...
def compute_green_on_fluence_surface(
    ...
    if source_radius is None:
        green_proj = green_infinite_point_source_on_surface(
            source_pos_physical, surface_coords, valid_mask, TISSUE_PARAMS
        )
    else:
...
def run_mcx_for_source(
    ...
    if source_kind == "point":
        pattern = np.zeros((11, 11, 11), dtype=np.float32)
        pattern[5, 5, 5] = 1.0
        pattern_shape = (11, 11, 11)
    elif source_kind == "ball":
    ...
    json_path = output_dir / f"m2_{source_kind}.json"
    ...
    jnii_path = output_dir / f"m2_{source_kind}.jnii"
...
def compute_metrics(mcx_proj: np.ndarray, green_proj: np.ndarray) -> dict:
    ...
    return {
        ...
        "mcx_sum": float(mcx_vals.sum()),
        "green_sum": float(green_vals.sum()),
    }


def main():
    parser = argparse.ArgumentParser(
        description="M2: Three-source single-view validation"
    )
    ...
    output_dir = (
        Path(args.output)
        if args.output
        else Path("pilot/paper04b_forward/mvp_pipeline/results/m2")
    )
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    source_configs = [
        ("point", None, None),
        ("ball", SOURCE_RADIUS, None),
        ("gaussian", None, SOURCE_SIGMA),
    ]

    for kind, radius, sigma in source_configs:
        logger.info(f"\n{'=' * 50}")
        ...
        if kind == "point":
            green_proj = compute_green_on_fluence_surface(
                fluence, SOURCE_POS_PHYSICAL, None
            )
        elif kind == "ball":
            green_proj = compute_green_on_fluence_surface(
                fluence, SOURCE_POS_PHYSICAL, radius
            )
        else:
            green_proj = compute_gaussian_green_on_fluence_surface(
                fluence, SOURCE_POS_PHYSICAL, sigma
            )
        ...
        logger.info(f"  NCC: {metrics['ncc']:.4f}")
        logger.info(f"  k:   {metrics['k']:.2e}")

        np.save(source_dir / "mcx_proj.npy", mcx_proj)
        np.save(source_dir / "green_proj.npy", green_proj)

    with open(output_dir / "m2_results.json", "w") as f:
        json.dump(results, f, indent=2)

    print(f"\n{'=' * 60}")
    print("M2 Results Summary")
    print(f"{'=' * 60}")
    print(f"{'Source':<12} {'NCC':<10} {'k':<15} {'Pass'}")
    print("-" * 60)
    for kind, data in results["sources"].items():
        passed = data["ncc"] >= 0.95 and 1e5 <= data["k"] <= 1e8
        print(
            f"{kind:<12} {data['ncc']:<10.4f} {data['k']:<15.2e} {'✓' if passed else '✗'}"
        )
    print(f"{'=' * 60}")
